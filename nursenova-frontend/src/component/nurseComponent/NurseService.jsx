...
import { addService, fetchServices, showServices } from "../../api/nurse";
...
const search = (e)=>{
  ...
  useEffect(()=>{
    const fetchService = async (nurse) => {
      try {
        const response =await fetchServices(nurse,currentPage - 1, pageSize,searchKey);
        ...
        // } else {
          const data ={
            ...
            dutyType: Object.fromEntries(
              Object.entries(dutyPrices).map(([key,value])=>[key,parseInt(value)])
            )
          };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <NurseServiceTable services={selectServices}/>
       ...
       <div className="space-y-6 p-6 bg-white rounded-lg shadow-xl max-w-md mx-auto">
        ...
        <form onSubmit={formSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700">Duty Types</label>
            {error && <p className="text-xs text-red-500 mt-1">Please select a duty type </p>}
                ...
                {selectDuty.includes(duty) && (
                  <div className="flex-grow">
                    <input
                      ...
                      className="w-full p-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-green-500 transition"
                    />
                    {errors[duty] && <p className="text-xs text-red-500 mt-1">{errors[duty]}</p>}
                  </div>
                )}
              </div>
            ))}
